...
youtube-dl --continue --max-quality=22 --ignore-errors --sub-lang en --user-agent "$USER_AGENT" --output "%(title)s (%(id)s).%(ext)s" "\$@"
EOF
...
youtube-dl --continue --max-quality=22 --ignore-errors --sub-lang en --user-agent "$USER_AGENT" --output "%(upload_date)s %(title)s (%(id)s).%(ext)s" "http://www.youtube.com/user/\$1"
EOF
...
youtube-dl --continue --max-quality=22 --ignore-errors --sub-lang en --user-agent "$USER_AGENT" --output "%(autonumber)s %(title)s (%(id)s).%(ext)s" "\$@"
EOF
...
count="\$1"
shift
youtube-dl --continue --max-quality=22 --ignore-errors --sub-lang en --user-agent "$USER_AGENT" --output "%(title)s (%(id)s).%(ext)s" "ytsearch\$count:\$@"
EOF
